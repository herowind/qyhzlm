...
class Attribute extends AuthController{
	...
	public function search() {
	    ...
	    $list = AllianceAttribute::where('pid',$pid)->order('sort desc')->select();
	    if($pid){
	        $pdetail = AllianceAttribute::get($pid);
	    }else{
	        $pdetail['title'] = '顶级';
	    }
        ...
        if($this->request->isPost()){
	        ...
	        if($checkRst !== true){
	            ...
	            if($this->request->param('id')){
	                //更新操作（先查询在更新）
	                $detail = AllianceAttribute::get($this->request->param('id'));
	                ...
	                //新增操作
	                $num = AllianceAttribute::create($data);
	                if($num !== false){
	                    $this->success('操作成功');
	                }else{
	                    $this->error('操作失败');
	...
	public function remove(){
	    $num = AllianceAttribute::destroy($this->request->param('id'));
	    if($num !== false){
	        $this->success('操作成功');
	    }else{
	        $this->error('操作失败');
